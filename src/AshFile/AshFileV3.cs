...
public partial class AshFile{
	protected internal class V3{
		#region read
		//Read
		public static Dictionary<string, object> Read(byte[] fileBytes, out AshFileFormatOptions conf){
			Dictionary<string, object> dic = new Dictionary<string, object>();
			
			ulong index = 5; //We start at 5 because 0 is version, and then %ASH comes
			AshFileFormatOptions config = new AshFileFormatOptions(fileBytes[0], fileBytes[index]) with {password = null};
			conf = config;
			index++;
			...
			if(config.compactBools){
				try{
					...
					throw new AshFileException("Error occured while reading V3", 3, e);
				}
			}
		...
		public static byte[] Write(IDictionary<string, object> d, AshFileFormatOptions conf){
			...
			try{
				...
				throw new AshFileException("Error occured while writing V3", 3, e);
			}
